...
import { extractSkillSchema } from '@/lib/validation/schemas'
...
export async function POST(request: NextRequest) {
  ...
  const parsed = extractSkillSchema.safeParse(body)
  if (!parsed.success) {
    return NextResponse.json({ error: 'Invalid input' }, { status: 400 })
  }

  const { jd_id, question, answer } = parsed.data
  ...
  try {
    const aiResponse = await getCompletion({
      ...
      userPrompt: buildSkillExtractionUserPrompt(question, answer),
      temperature: 0.3,
      maxTokens: 256,
    })
  ...
  if (typeof result.skill_identified !== 'string' || typeof result.resume_bullet !== 'string') {
