            ...
            <div className="flex flex-col items-center gap-1">
              <div
                className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-medium
                  ${isDone ? 'bg-black text-white' : isCurrent ? 'bg-black text-white' : 'bg-gray-200 text-gray-500'}`}
              >
                {isDone ? '✓' : step}
              </div>
              <span className={`text-xs ${isCurrent ? 'font-medium text-black' : 'text-gray-400'}`}>{label}</span>
            </div>
            {step < STEPS.length && <div className={`h-px w-6 sm:w-12 ${isDone ? 'bg-black' : 'bg-gray-200'}`} />}
          </div>
        )
      })}
    </div>
